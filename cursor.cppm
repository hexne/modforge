...
export class Cursor {
    ...
    CursorPos old_cursor_pos_, cursor_pos_;
...
public:
    Cursor();
    explicit Cursor(CursorPos);
    ~Cursor();

    void click_left(int count);
    void click_right(int count);
    void wheel_up(int val);
    void wheel_down(int val);
...
Cursor::Cursor(const CursorPos cursor_pos) : cursor_pos_(cursor_pos) {
...
void Cursor::click_right(int count = 1) {
    while (count--)
        click_right(cursor_pos_);
}
void Cursor::wheel_up(int val = 100) {
    mouse_event(MOUSEEVENTF_WHEEL, 0, 0, val, 0);
}
void Cursor::wheel_down(int val = -100) {
    mouse_event(MOUSEEVENTF_WHEEL, 0, 0, val, 0);
}
...
void Cursor::listen_event() {
    ...
    while (true) {
        ...
        if (const auto cur_cursor_pos = get_cursor_pos(); cur_cursor_pos != old_cursor_pos_) {
            old_cursor_pos_ = cursor_pos_ = cur_cursor_pos;
